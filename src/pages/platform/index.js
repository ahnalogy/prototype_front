...
const PlatformRegister = () => {
  ...
  const [refplatforms, setRefPlatforms] = useState([]);
  ...
  const loadPlatforms = async () => {
    try {
      ...
      const { ok: refOk, data: refData } = await fetchRefPlatformList();
      if (refOk) {
        setRefPlatforms(refData);
        console.log("참조 플랫폼 목록 설정 완료:", refData);
        if (refData && refData.length > 0) setPlatformName(refData[0].name);
      }
    } catch (error) {
